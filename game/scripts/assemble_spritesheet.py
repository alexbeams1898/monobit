...
def main():
    ...
    # Skeleton universal sheet. Look in $TEMP or /tmp.
    ...
    if not skip_enemy:
        if os.path.exists(skeleton_path):
            print("Assembling skeleton sprite sheet...")
            assemble_from_universal_sheet(
                skeleton_path,
                os.path.join(sprites_dir, "skeleton_sheet.png"),
            )
        else:
            print(f"  WARNING: Skeleton sheet not found at {skeleton_path}")
            print("  Falling back to red-tinted humanoid for skeleton sheet.")
            assemble_lpc_sheet(
                lpc_dir,
                os.path.join(sprites_dir, "skeleton_sheet.png"),
                ALL_LAYERS,
