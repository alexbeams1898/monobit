...
Adapted from the sibling prison-escape-game project. Mono additions:
  - "Lore" category for narrative-design changes (docs/design/ ships
    through the same review process as code; see PR template).
  - REPO_ROOT resolves to the parent of scripts/ (mono layout) rather
    than the parent of game/scripts/ (prison-break layout).
  - PUBLIC_RELEASES_REPO points at the source repo (mono is private; no
    mirror repo yet — release.yml is deferred until v0.1).
      ...
      (b) One or more bullets, each matching:
          ...
          where:
              <Category> is one of: Added, Changed, Deprecated, Removed,
                                    Fixed, Security, Performance, Lore
...
# Mono is private; no separate releases-mirror repo yet. Points at the
# source repo. When release.yml lands and we set up a public mirror,
# update this constant in lockstep with release.yml.
SOURCE_REPO = "alexbeams1898/mono"

# scripts/changelog.py is at the repo root + 1 level deep.
...
def cmd_release(version: str) -> None:
    ...
    new_link = f"[{version}]: https://github.com/{SOURCE_REPO}/releases/tag/v{version}"
    unreleased_link = f"[unreleased]: https://github.com/{SOURCE_REPO}/compare/v{version}...HEAD"
