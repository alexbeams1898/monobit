...
- **unlock::Condition** — the shared gate primitive: a clause ANDs `observed` / `flag` / `stat` / `carrying`, plus `without` (the NEGATIVE half -- an id that must NOT be held, prefixed `item:`/`obs:`/`flag:`) and WHEN (`between` for an hours-window that may wrap midnight, `day_min`/`day_max` for a stretch of days), and clauses OR. Reused by observation tiers, deeds, thoughts, schedules, and visibility gates (crafting does NOT gate — see crafting) — [include/UnlockCondition.h](../include/UnlockCondition.h)
...
- **psyche** — the one content/cognition engine. An **Encounter** is a glowing spot you can OBSERVE (reading tiers) or ACT ON (deeds) -- it must offer BOTH to load, though the player may lean entirely on one. Plus thoughts (written) and remarks (spoken), four line kinds by initiator/voice (observation = chosen, impression = pressed, remark = speech, thought = concluded), the ambient re-check engine, per-spot taken-state, `{player}` name binding, and THE NOTEBOOK RULE (a thought is where he articulates what he never says aloud: without one carried, a thought waits unfired and a want line surfaces on every blocked attempt; it lands when he returns to the thing with one). Deeds may grant items / consume their spot (drops), and carry authored `minutes` — [include/Psyche.h](../include/Psyche.h)
- **npc** — a being on the map (person or animal): config (WHO: art, name, walk feel, named anims) + LDtk placement (WHERE) + speaker-flagged psyche content (WHAT -- an animal is an npc that never speaks); spawn/steer helpers scenes reuse. Ambient LIFE = routines (looping wander/move_to/face/anim/wait steps, collision-checked) selected by a clock-driven, flag-gated schedule ("HH:MM" windows; the calendar grows inside worldclock) — [include/Npc.h](../include/Npc.h)
...
- **inventory** — the satchel: two-layer item model (blueprint / instance), categories, ops (add/remove/count/has), key-item gating, new-find flag. Item ART is a cell on the shared items sheet (`icon` + `icon_cell`, built by tools/tileset/pack_items.py) so the floor sprite and the satchel chip cannot disagree — [include/Inventory.h](../include/Inventory.h)
- **yields** — what a placed thing gives up when it gives more than one certain thing: weighted entries with quantity ranges, drawn from on the take (a pile of storm wood gives branches and now and then a stone). A pickup names either one `item` or a `yields` table — [include/Yields.h](../include/Yields.h)
...
- **world_items** — things lying in the world: floor sprites + actionable-only interactables, with a rim-outline cue. ONE placed thing whose YIELD is a field (`item` = exactly that, `yields` = a draw from that table) — the entity's name is never read. A placement may carry its OWN art (the tileset region its entity shows in the editor), so a pile of storm wood looks like itself rather than like a stand-in; else it draws the item's icon, else the configured stand-in. `group` + `clears_flag` make several placements one CLEARING: emptying it raises the flag — [include/WorldItems.h](../include/WorldItems.h)
- **arcs / the agenda** — a thread: one goal flag reached by any of several ROUTES. The routes are authoring apparatus (checked at boot against what the content can produce, never read in play); an arc that also carries a `line` is an ERRAND and surfaces as the agenda at the head of today's notebook page. On the list = learned (`known_when`) and unfinished (`goal_flag` unset) — no accepted/active/failed state to disagree with the world. A `window` (the same `between`/`day_min` clause grammar) says whether the world will receive it now, and a shut window never fails a thread, only says "not now" — [include/Arcs.h](../include/Arcs.h)
...
- **glimmer** — the encounter highlight: a beating rim (the shared Outline component) on the ART itself -- the prop under the box, or, for a person's encounter, that person's body by name, whose box travels with them. Lit only while it is the active target (in reach AND faced, or hovered); brightness is a flat dial or the Perception formula (config switch) — [include/Glimmer.h](../include/Glimmer.h)
...
- **spirit_hud** — the Spirit total in the corner, and the "+N" a gain announces itself with before FEEDING INTO it: the shown number climbs to the real one, so a big understanding reads as a long climb rather than a digit changing (the souls-counter model). Replaces the old "+N Spirit" toast — [include/SpiritHud.h](../include/SpiritHud.h)
