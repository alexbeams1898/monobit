...
Inputs:
  ...
  --sample-rate HZ    conform output to this rate (default: 44100 -- wayworn SFX)
  --channels N        conform output to this channel count (default: 1 = mono)
...
Output is ogg/vorbis (the engine's preferred SFX format), conformed to a
single sample rate + channel count at decode time so a source in any
format yields a bank matching the game's SFX convention (44100 mono).

Requires ffmpeg in PATH. Self-contained (helpers inlined) so this game
takes no dependency on another game's scripts directory -- the same
copy-not-import convention prison-escape -> selva-oscura already follows.
"""
...
def decode_to_wav(input_path, wav_path, sample_rate=None, channels=None):
    """Decode any ffmpeg-supported format to 16-bit PCM WAV, optionally
    resampling to sample_rate and/or downmixing to `channels`. Conforming
    here (not at encode) means onset detection + slicing run at the target
    rate, so slice lengths in frames map to the output rate directly."""
    cmd = ["ffmpeg", "-y", "-i", input_path, "-acodec", "pcm_s16le"]
    if sample_rate:
...
def chop(source_path, out_dir, prefix, max_variations, slice_seconds,
         ...
         sample_rate=None, channels_out=None):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        decode_to_wav(str(src), tmp_wav, sample_rate, channels_out)
...
def main():
    ...
    ap.add_argument("--sample-rate", type=int, default=44100,
                    help="conform output to this sample rate (wayworn SFX = 44100)")
    ap.add_argument("--channels", type=int, default=1,
                    help="conform output to this channel count (1 = mono)")
    args = ap.parse_args()
    chop(args.source, args.out_dir, args.prefix, args.max_variations,
         ...
         args.min_gap_seconds, args.sample_rate, args.channels)
