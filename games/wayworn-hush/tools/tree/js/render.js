...
export function render() {
  ...
  const matches = (n) => !q || (n.kind === "flag" ? n.name
    : n.kind === "chip" ? n.label
  ...
  graph.edges.forEach(e => {
    e._forward = pos[e.to].x >= pos[e.from].x + widthOf[e.from];
    (faces[e.from + "|" + (e._forward ? "R" : "L")] ||= []).push({ e, start: true, far: e.to });
    (faces[e.to + "|" + (e._forward ? "L" : "R")] ||= []).push({ e, start: false, far: e.from });
  });
  const heightOf = (key) => {
    const seats = Math.max((faces[key + "|L"] || []).length, (faces[key + "|R"] || []).length);
    return Math.max(NODE_H[kindOf[key] || "enc"], seats * 14 + 8);
  };
  ...
  graph.edges.forEach(e => {
    ...
    if (!a || !b) return;
    // Side-aware routing: a forward edge runs right-side -> left-side; a BACKWARD
    // edge (a thought deepening an earlier encounter's tier) exits the source's
    // LEFT and enters the target's RIGHT -- a short local hook instead of a long
    // sweep across the whole graph. Endpoints sit in their face's seats.
    const x1 = e._forward ? a.x + widthOf[e.from] : a.x;
    const x2 = e._forward ? b.x : b.x + widthOf[e.to];
    const y1 = seatY(e.from, e._forward ? "R" : "L", e, true);
    const y2 = seatY(e.to, e._forward ? "L" : "R", e, false);
    const mx = (x1 + x2) / 2;
    ...
    p.setAttribute("marker-end", `url(#arrow-${e.cls === "linking" ? "obs" : e.cls})`);
  ...
  graph.nodes.forEach(n => {
    ...
    if (k) {
      if (n.kind === "enc") {
        ...
        badge = lit ? "landed" : (eligible ? "can land" : "locked");
      } else if (n.kind === "chip") {
        lit = condMet(n.cond, k);
        badge = lit ? "open" : "locked";
        unreachable = !lit;
      } else {
        lit = k.flags.has(n.name);
        badge = lit ? "set" : "unset";
      }
    ...
    const fill = n.kind === "flag" ? "var(--flag)"
      : n.kind === "thought" ? "var(--thought)"
    ...
    if (n.kind === "flag") {
      inner += `<text x="10" y="19">⚑ ${esc(trunc(n.name, 18))}</text>`;
    } else if (n.kind === "chip") {
...
function renderSimPanel() {
  ...
  p.innerHTML = "<h3>Simulate: stats</h3>" + (stats.length ? stats.map(s => `
    ...
    `<h3 style="margin-top:8px">click nodes to toggle</h3>
     <span class="dimtext">encounter = observed · thought = landed (auto-sets its
     flag) · flag = set by hand · chips just show open/locked</span>
