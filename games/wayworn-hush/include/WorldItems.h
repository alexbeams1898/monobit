...
// scene the way a real object would. Interacting takes it directly -- the
// interactable is actionable-only (no reading). A gather node (a harvest spot backed by a
// yield table) renders a configurable stand-in sprite until node art is authored. See
...
// and rolls its yield table. Y-sorted by its base so the player draws in front/behind by
